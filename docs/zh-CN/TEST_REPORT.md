...
| -------- | ------------------------------ |
| 库版本   | 1.1.0                          |
| 测试框架 | @dreamer/test@1.2.0            |
| 测试日期 | 2026-07-22                     |
| 测试环境 | Deno 2.x / Bun 1.x / Node 22.x |
...
| 指标     | 值                                         |
| -------- | ------------------------------------------ |
| 总用例数 | 71（Bun/Node）/ 72（Deno）                 |
| 通过     | 71（Bun/Node）/ 72（Deno）                 |
| 失败     | 0                                          |
| 通过率   | 100%                                       |
| 执行时间 | ~6ms（Deno）/ ~36ms（Bun）/ ~460ms（Node） |

> 三端验证（2026-07-22）：`deno test -A tests/` → 72 通过；`bun test tests/` →
> 71 通过；`npm run test:node`（`tsx --test`）→ 71 通过。Deno 多出的 1 项为
> 框架隐式的 `@dreamer/test cleanup browsers` 清理用例。三端均 0 失败。

### 测试文件统计

| 测试文件    | Deno | Bun | Node | 状态    |
| ----------- | ---- | --- | ---- | ------- |
| mod.test.ts | 72   | 71  | 71   | ✅ 通过 |
