...
- **`formatRelative` 未来时间测试在 Windows CI 上 flaky**：测试
  `未来时间应该使用'后'` 用 `Date.now() + 5 * 60 * 1000`（精确 5 分钟边界）。
  `formatRelative` 内部 `Math.floor(absDiff / MINUTE)`，时间戳创建与内部
  `Date.now()` 间任一 tick 延迟都会使 `absDiff` 变 299999ms → 向下取整为 4 → "4
  分钟后" 而非 "5 分钟后"。添加 30 秒缓冲（`(5 * 60 + 30) * 1000`），使测试
  在所有 CI runner 上确定性通过。
...
- **`@dreamer/test`**：`^1.2.1` → `^1.2.3`（deno.json 与 package.json
